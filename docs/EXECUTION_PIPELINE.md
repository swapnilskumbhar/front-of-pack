...
Current verification snapshot (2026-08-23): local tests prove bounded local JPEG/PNG/WebP dimension validation, original-image preservation, one-call analysis, authorization, profiles and zero-call cache hits. Production proves public TLS, R2/D1/Queues/Jobs/Terra, cache hits, and live WhatsApp webhook/media/analysis/delivery with terminal ciphertext cleanup. Prompt v5 transcribes structured nutrition while decision-engine v1 independently derives whole-pack serving reality from printed values and prioritizes it above generic model findings. FSSAI, Legal Metrology, CDSCO, experimental INR, FoSCoS, BIS Care and NCH remain allow-listed. Exact registry and editable no-submit grievance are public. Tests stand at 48/48 root and 25/25 Jobs Worker cases. Real-product regression, officer rehearsal and final submission evidence remain.
