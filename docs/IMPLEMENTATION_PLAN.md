...
Current proof (2026-08-24): production proves one-call analysis, hosted search and live WhatsApp delivery. Prompt v9 transcribes structured package facts plus web-match confidence and requires provisional analysis of sufficiently matched online ingredients, nutrition and claims rather than link repetition. Decision-engine v3 adds high whole-container signals even when one serving equals the container, dietary/allergen synthesis, and a four-level consumer indicator. Web and WhatsApp show a punchy indicator, profile, confidence and at most two key reasons while preserving expandable/source-linked evidence. Engine version is persisted and included in cache identity. `/how-we-decide` publishes the logic. Real Haldiram and English Oven demos include photographs; the cart fixture remains explicitly synthetic until a real image is supplied.
