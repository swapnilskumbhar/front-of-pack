...
Current proof (2026-08-24): local and production runs validate encoded dimensions while preserving original bytes. Production has proved one-call analysis, capability polling, cache hits, profiles and live WhatsApp delivery. Prompt v8 transcribes structured nutrition, exact printed claims, ingredient tokens and a visible veg/non-veg mark. Decision-engine v2 derives whole-pack impact, conservative literal contradictions, explicit dietary origin, source-unclear additives, Jain preference information and FSSAI allergens. Engine version is persisted and included in cache identity. An exact Haldiram's front-only fixture proved hosted search and official product-page matching. `/how-we-decide` publishes the engine tables. Real Haldiram and English Oven cached demos include their photographs; the six-product fixture remains explicitly synthetic until a real cart image is supplied.
